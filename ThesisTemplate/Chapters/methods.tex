...
\marginnote{\newthought{Interfaces before internals.} Define what goes in and what comes out first, in which format and against which contract. Only then design the system behind those interfaces. Fixed boundaries let the inside change without breaking everything around it.}
...
\marginnote{\newthought{Motivate every design decision} by exactly one of three: the state of the art, meaning someone has shown it works; your own experiment, meaning you measured it here; or a boundary condition, meaning data, compute, time, or your partner decided it. An unmotivated choice is the first thing an examiner finds.}
